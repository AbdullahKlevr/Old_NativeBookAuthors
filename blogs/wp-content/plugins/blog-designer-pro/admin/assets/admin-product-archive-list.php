...
if ( isset( $_GET['list'] ) ) {
	...
	$total        = $wpdb->get_var( 'SELECT COUNT(`id`) FROM ' . $wpdb->prefix . 'bdp_product_archives ' . $where );
	$num_of_pages = ceil( $total / $limit );
}
...
if ( isset( $_GET['list'] ) ) {
	...
	if ( 'all' === $_GET['list'] ) {
		$archives = $wpdb->get_results( 'SELECT * FROM ' . $wpdb->prefix . 'bdp_product_archives ' . $where . ' order by ' . $order_field . ' ' . $order_by . ' limit ' . $offset . ', ' . $limit );
	} else {
		$archives = $wpdb->get_results( 'SELECT * FROM ' . $wpdb->prefix . 'bdp_product_archives ' . $where . $condition . ' order by ' . $order_field . ' ' . $order_by . ' limit ' . $offset . ', ' . $limit );
	}
} else {
	$archives = $wpdb->get_results( 'SELECT * FROM ' . $wpdb->prefix . 'bdp_product_archives ' . $where . ' order by ' . $order_field . ' ' . $order_by . ' limit ' . $offset . ', ' . $limit );
}

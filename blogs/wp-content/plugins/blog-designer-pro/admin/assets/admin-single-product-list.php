...
$total        = $wpdb->get_var( 'SELECT COUNT(`id`) FROM ' . $wpdb->prefix . 'bdp_single_product ' . $where );
...
$shortcodes = $wpdb->get_results( 'SELECT * FROM ' . $wpdb->prefix . 'bdp_single_product ' . $where . ' order by ' . $order_field . ' ' . $order_by . ' limit ' . $offset . ', ' . $limit );
?>
	...
	<form method="post">
		...
		<div class="tablenav top">
			...
			>
				...
				<?php
				echo esc_html( number_format_i18n( $total ) ) . ' ' . sprintf( _n( 'item', 'items', $total, 'blog-designer-pro' ), esc_html( number_format_i18n( $total ) ) );
				?>
				</span>

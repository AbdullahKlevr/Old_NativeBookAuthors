...
class Bdp_Woocommerce {
	...
	public function woocommerce_product_images( $bdp_settings, $bdp_post_id ) {
		...
		if ( 'product' === $post_type && isset( $bdp_settings['display_sale_tag'] ) && 1 == $bdp_settings['display_sale_tag'] ) {
			echo '<div class="bdp_woocommerce_sale_wrap '. $bdp_settings['bdp_sale_tagtext_alignment'] .'">';
	...
	public static function get_single_product_template_settings() {
		...
		$single_template          = $wpdb->get_row( 'SELECT * FROM ' . $wpdb->prefix . 'bdp_single_product WHERE find_in_set("' . $post_id . '",single_product_id) <> 0' );
		$single_category_template = '';
		$single_tag_template      = '';
		if ( $cat_ids ) {
			foreach ( $cat_ids as $cat_id ) {
				if ( is_numeric( $cat_id ) ) {
					$category_template = $wpdb->get_row( 'SELECT id FROM ' . $wpdb->prefix . 'bdp_single_product WHERE single_product_template = "category" AND find_in_set("' . $cat_id . '",sub_categories) <> 0' );
				}
		...
		if ( $tag_ids ) {
			foreach ( $tag_ids as $tag_id ) {
				if ( is_numeric( $tag_id ) ) {
					$tag_template = $wpdb->get_row( 'SELECT id FROM ' . $wpdb->prefix . 'bdp_single_product WHERE single_product_template = "tag" AND find_in_set("' . $tag_id . '",sub_categories) <> 0' );
				}

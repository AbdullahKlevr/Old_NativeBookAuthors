...
class Bdp_Front_Functions {
	...
	public function bdp_front_stylesheet() {
		...
		if ( ! is_admin() ) {
			...
			wp_enqueue_style( 'bdp-bookblock-css', plugins_url( 'css/bookblock.css', __FILE__ ) );
		...
		if ( isset( $bdp_settings_array ) && is_array( $bdp_settings_array ) && ! empty( $bdp_settings_array ) ) {
			...
			foreach ( $bdp_settings_array as $shortcode_id => $bdp_settings ) {
				if ( 'crayon_slider' === $bdp_settings['template_name'] || 'sunshiny_slider' === $bdp_settings['template_name'] || 'sallet_slider' === $bdp_settings['template_name'] || 'colorful_sliding' === $bdp_settings['template_name'] || 'blog_carousel' === $bdp_settings['template_name'] || 'threed_carousel' === $bdp_settings['template_name'] || 'flip_book_3d' === $bdp_settings['template_name'] ) {
					...
					if ( 'threed_carousel' === $bdp_settings['template_name'] ) {
	...
	public function bdp_shortcode_function( $atts ) {
		...
		if ( is_numeric( $atts['id'] ) ) {
			$settings_val = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}blog_designer_pro_shortcodes WHERE bdid = %d", $atts['id'] ), ARRAY_A );
		}
		if ( ! $settings_val ) {
				...
				if ( is_tax( 'download_category' ) && in_array( 'category_template', $download_archive_list ) ) {
					...
					foreach ( $download_archive_list as $archive ) {
						if ( 'category_template' === $archive ) {
							global $wpdb;
							if ( is_numeric( $category_id ) ) {
								$category_template = $wpdb->get_row( 'SELECT * FROM ' . $wpdb->prefix . 'bdp_edd_archives WHERE download_archive_template = "category_template" AND find_in_set("' . $category_id . '", download_sub_categories) <> 0' );
							}
							if ( ! empty( $category_template ) ) {
				...
				if ( is_tax( 'download_tag' ) && in_array( 'tag_template', $download_archive_list ) ) {
					...
					foreach ( $download_archive_list as $archive ) {
						if ( 'tag_template' === $archive ) {
							global $wpdb;
							if ( is_numeric( $category_id ) ) {
								$category_template = $wpdb->get_row( 'SELECT * FROM ' . $wpdb->prefix . 'bdp_edd_archives WHERE download_archive_template = "tag_template" AND find_in_set("' . $category_id . '", download_sub_categories) <> 0' );
							}
							if ( ! empty( $category_template ) ) {
								$template = get_stylesheet_directory() . '/bdp_templates/edd_templates/archive/archive-download.php';
								if ( ! file_exists( $template ) ) {
		...
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'ajax-nonce' ) ) {
			...
			if ( 'blog_layout' === $layout ) {
				...
				if ( is_numeric( $blog_shortcode_id ) ) {
					$settings_val = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}blog_designer_pro_shortcodes WHERE bdid = %d", $blog_shortcode_id ), ARRAY_A );
					$allsettings  = $settings_val[0]['bdsettings'];
				}
				...
				if ( is_numeric( $blog_shortcode_id ) ) {
					$settings_val = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}bdp_archives WHERE id = %d", $blog_shortcode_id ), ARRAY_A );
				...
				$settings_val      = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}bdp_product_archives WHERE id = %d", $blog_shortcode_id ), ARRAY_A );
				$allsettings       = $settings_val[0]['settings'];
				if ( is_serialized( $allsettings ) ) {
					$bdp_settings = maybe_unserialize( $allsettings );
				}
				...
				$settings_val      = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}bdp_edd_archives WHERE id = %d", $blog_shortcode_id ), ARRAY_A );
				$allsettings       = $settings_val[0]['settings'];
				if ( is_serialized( $allsettings ) ) {
					$bdp_settings = maybe_unserialize( $allsettings );
				}
	...
	public function bdp_email_share_form() {
		...
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'ajax-nonce' ) ) {
			...
			if ( 'date' === $cur_page || 'author' === $cur_page || 'category' === $cur_page || 'tag' === $cur_page ) {
				...
				$settings_val     = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}blog_designer_pro_shortcodes WHERE bdid = %d", $txt_shortcode_id ), ARRAY_A );
				if ( $settings_val ) {
		...
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'ajax-nonce' ) ) {
			...
			if ( 'blog_layout' === $layout ) {
				...
				if ( 'boxy' === $bdp_theme || 'brit_co' === $bdp_theme || 'glossary' === $bdp_theme || 'invert-grid' === $bdp_theme ) {
					echo "<div class='bdp-row $bdp_theme $same_height_class'>";
				}
				...
				if ( 'media-grid' === $bdp_theme || 'chapter' === $bdp_theme || 'roctangle' === $bdp_theme || 'glamour' === $bdp_theme || 'famous' === $bdp_theme || 'minimal' === $bdp_theme ) {
					...
					if ( 'roctangle' === $bdp_theme ) {
						...
						echo '<div class="bdp-row '. $column_class. ' ' . $bdp_theme . ' ' . $same_height_class . '">';
					}
				}
				if ( 'wise_block' === $bdp_theme ) {
					echo '<div class="blog_template wise_block_wrapper ' . $same_height_class . ' ' . $bdp_theme . '">';
				}
				...
				echo '<input type="hidden" name="found_posts" id="found_posts" value="' . $found_posts . '" />';
				...
				echo '<input type="hidden" name="blog_shortcode_id" id="blog_shortcode_id" value="' . $blog_shortcode_id . '" />';
				echo '<input type="hidden" name="terms" id="terms" value="' . $filter_terms . '" />';
				if ( 1 == $bdp_settings['display_filter'] ) {
					echo '<input type="hidden" name="filter_type" id="filter_type" value="' . $bdp_settings['display_filter_by'] . '" />';
